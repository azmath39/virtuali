...
class User < ActiveRecord::Base
  ...
  def package=(pkg)
      ...
      self.selected_package=SelectedPackage.create(:package_id=>p.id,:price=>p.yearly_price,:pictures_for_tour=>p.pictures_for_tour,:payment_period_type=>pkg["type_of_payment"])
      else
      self.selected_package=SelectedPackage.create(:package_id=>p.id,:price=>p.yearly_price,:pictures_for_tour=>p.pictures_for_tour)
      end
  end
  def product=(pro)
    unless pro.nil?
